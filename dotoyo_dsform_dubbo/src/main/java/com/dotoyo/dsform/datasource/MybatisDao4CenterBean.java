...
public class MybatisDao4CenterBean extends SqlSessionFactoryBean {
	protected static final transient IFrameLog log = new LogProxy(
			LogFactory.getLog(MybatisDao4CenterBean.class));
	public MybatisDao4CenterBean(){
		log.debug("MybatisDao4CenterBean");
	}
	@Override
				...
				if (map != null) {
					...
					if (!StringsUtils.isEmpty(dbType)) {
						String url = String.format(
								"classpath*:mybatis/%s/com/dotoyo/busi/**/*.xml", dbType);
						PathMatchingResourcePatternResolver r = new PathMatchingResourcePatternResolver();
						try {
	...
	protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
		...
		FrameSqlSessionFactory f=new FrameSqlSessionFactory(factory);
		return f;
	}
}
