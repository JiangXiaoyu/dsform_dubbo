...
import com.dotoyo.ims.dsform.allin.UserLoginBo;
 ...
 * 
 */
public class SpringDataSource4CenterBean implements DataSource {
	protected static final transient IFrameLog log = new LogProxy(
			LogFactory.getLog(SpringDataSource4CenterBean.class));
	private String code = "";
	...
	private SpringDataSource4CenterBean() {
		//log.debug("1");
	}
	...
	public Connection getConnection(String username, String password)
		...
		try {
			...
			return FrameFactory.getDataSourceFactory(null).getDataSource(
					getDsCode()).getConnection(username, password);
		} catch (SQLException e) {
			throw e;
		} catch (Throwable e) {
			throw new SQLException(e.getMessage());
	 ...
	 * @throws SQLException
	 */
	protected String getDsCode1() throws SQLException {
		String sub = "";
		try {
			IFrameAuthority auth = (IFrameAuthority) FrameFactory
					.getAuthorityFactory(null);

			UserLoginBo bo = auth.getLoginUser(null);
			if (bo != null) {
				sub = bo.getCenter();
			}
		} catch (Throwable e) {
