...
public class ProjectWidge implements IWidgeType {

	public static final String WIDGE_NAME = "project".toLowerCase();

	private IWidgeType widge = null;

	public ProjectWidge(IWidgeType widge) {
		this.widge = widge;
	}
					...
					.append(String
							.format(
									"<div style='width: %spx; height: %spx; border: 0px solid #ccc;'>"
											...
											+ " text-align:%s; vertical-align: bottom; word-wrap: break-word; word-break: break-all;%s' size='%s' onfocus='toolBarFocusEvent(this);' onblur='toolBarBlurEvent(this);' oninput='changeEvent(this);' onpropertychange='changeEvent(this);' title='自动获取机构名'></div></div>",
									width, height, tagId, tagId, WIDGE_NAME,
									"orgPro", width, height, textAlign,
									fontStyle, length / 2));
			return WIDGE_NAME;
		} else {
			if(widge == null){
