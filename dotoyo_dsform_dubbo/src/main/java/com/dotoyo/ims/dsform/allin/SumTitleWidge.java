 ...
 *
 */
public class SumTitleWidge implements IWidgeType {

	public String WIDGE_NAME = "sumTitle".toLowerCase();

	private IWidgeType widge = null;
	
	private ExcelTableBo table = null;

	public SumTitleWidge(IWidgeType widge,ExcelTableBo table) {
		this.widge = widge;
		this.table = table;
	}
	...
	public String append2EditHtml(String widgeType, StringBuilder sb, float width,
		...
		if (isCurWid(widgeType)) {
			...
			table.setTotalOfSumTitle(Integer.parseInt(table.getTotalOfSumTitle()) + 1 + "");
			return WIDGE_NAME;
		} else {
			if(widge == null){
				return "";
