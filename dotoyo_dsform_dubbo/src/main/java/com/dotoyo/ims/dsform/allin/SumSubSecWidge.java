 ...
 *
 */
public class SumSubSecWidge implements IWidgeType {

	public String WIDGE_NAME = "sumSubSec".toLowerCase();

	private IWidgeType widge = null;
	
	private ExcelTableBo table = null;

	public SumSubSecWidge(IWidgeType widge,ExcelTableBo table) {
		this.widge = widge;
		this.table = table;
	}
	...
	public String append2EditHtml(String widgeType, StringBuilder sb, float width,
		...
		if (isCurWid(widgeType)) {
			...
			table.setTotalOfSumSubSec( Integer.parseInt(table.getTotalOfSumSubSec()) + 1 + "");
			return WIDGE_NAME;
		} else {
			if(widge == null){
				return "";
