 ...
 *
 */
public class ItemnWidge implements IWidgeType {


	private IWidgeType widge = null;

	public ItemnWidge(IWidgeType widge) {
		this.widge = widge;
	}
	...
	public boolean isCurWid(String widgeType){
		Pattern pattern = Pattern.compile("itemn\\d+");
		if(pattern.matcher(widgeType.toLowerCase()).matches() || widgeType.toLowerCase().equals("itemn")){
			return true;
		}
		return false;
