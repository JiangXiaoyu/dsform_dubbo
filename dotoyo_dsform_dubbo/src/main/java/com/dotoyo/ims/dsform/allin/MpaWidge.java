...
//强度值MPA
public class MpaWidge implements IWidgeType {
	private static final String WIDGE_NAME = "mpa".toLowerCase();
	private IWidgeType nextWidge = null;

	public MpaWidge(IWidgeType widge) {
		...
		if (isCurWid(widgeType)) {
			...
			sb.append(String
					.format(
							...
							"text-align:%s; vertical-align: middle; word-wrap: break-word; word-break: break-all; background-color:%s; %s' oninput='mpaChangeEvent(this);' onpropertychange='mpaChangeEvent(this);' onkeyup='mpaChangeEvent(this);' onfocus='toolBarFocusEvent(this);' onblur='toolBarBlurEvent(this);' size='%s'></div></div>",
							width, height, tagId, tagId, WIDGE_NAME, width, height, textAlign, PreformConstant.BACKGROUND_COLOR_3, fontStyle, length/2));
			return WIDGE_NAME;
		} else {
			if(nextWidge == null){
				return "";
