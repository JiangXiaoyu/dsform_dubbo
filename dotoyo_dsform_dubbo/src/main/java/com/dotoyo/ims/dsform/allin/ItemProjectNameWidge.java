 ...
 *
 */
public class ItemProjectNameWidge implements IWidgeType {

	public static final String WIDGE_NAME = "itemProjectName".toLowerCase();

	private IWidgeType widge = null;

	public ItemProjectNameWidge(IWidgeType widge) {
		this.widge = widge;
	}
					...
					.append(String
							.format(
									"<div style='width: %spx; height: %spx; border: 1px solid #ccc;'>"
											...
											+ " text-align:%s; vertical-align: bottom; word-wrap: break-word; word-break: break-all;background-color:%s;' size='%s' onfocus='toolBarFocusEvent(this);' onblur='toolBarBlurEvent(this);' oninput='changeEvent(this);' onpropertychange='changeEvent(this);' title='自动获取单位工程名称'></div></div>",
			...
			sb
					.append(String
							.format(
									...
									"WIDGE_NAME", width, height, textAlign,
									fontStyle, length / 2));
			return WIDGE_NAME;
		} else {
			if(widge == null){
