 ...
 *  item--分项控件
 */
public class ItemWidge implements IWidgeType {
	public static final String WIDGE_NAME = "item".toLowerCase();
	private IWidgeType nextWidge = null;

	public ItemWidge(IWidgeType widge) {
		...
		if (isCurWid(widgeType)) {
			...
			sb.append(String
					.format(//contenteditable='true' showmenu='true'必须在一起
							...
							"<div id='%s' name='%s' widgeType='%s' isTextarea='true' contenteditable='true' value='%s' showmenu='true' style='width: %spx; height: %spx; display: table-cell;"+
	...
	public boolean isCurWid(String widgeType){
		if((widgeType.toLowerCase().equals(WIDGE_NAME) || widgeType.toLowerCase().startsWith(WIDGE_NAME+"[" )) && !widgeType.equals("itemnum")){
			return true;
		}
		return false;
	}
